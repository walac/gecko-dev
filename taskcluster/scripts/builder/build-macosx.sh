...
: TOOLTOOL_CACHE                ${TOOLTOOL_CACHE:=$TMPDIR}
...
export UPLOAD_PATH=${UPLOAD_PATH:=$(pwd)/build/artifacts}

set -v

fail() {
    echo # make sure error message is on a new line
    echo "[build-macosx.sh:error]" "${@}"
    exit 1
}
...
/tools/buildbot/bin/python build/src/testing/${MOZHARNESS_SCRIPT} ${config_cmds} \
  $debug_flag \
  $custom_build_variant_cfg_flag \
  --disable-mock \
  $actions \
